...
import com.adrielc.freearrow.FreeArrowLike.FreeArrowChoiceLike
...
object FreeArrowChoice { self =>
  ...
  final private[freearrow] case class Id[A]() extends FAC[Nothing, A, A] {
    def foldMap[G[_, _]](fk: Nothing ~~> G)(implicit A: ArrowChoice[G]): G[A, A] = A.id
  }
  final private[freearrow] case class Pure[A, B](f: A => B) extends FAC[Nothing, A, B] {
  ...
  final private[freearrow] case class Lift[F[_, _], A, B](fab: F[A, B]) extends FAC[F, A, B] {
    def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[A, B] = fk(fab)
  }
  final private[freearrow] case class Merge[F[_, _], A, B, C](_first: FAC[F, A, B], _second: FAC[F, A, C]) extends FAC[F, A, (B, C)] {
    def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[A, (B, C)] = A.merge(_first.foldMap(fk), _second.foldMap(fk))
  }
  final private[freearrow] case class Split[F[_, _], A, B, C, D](_first: FAC[F, A, B], _second: FAC[F, C, D]) extends FAC[F, (A, C), (B, D)] {
    override def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[(A, C), (B, D)] = A.split(_first.foldMap(fk), _second.foldMap(fk))
  }
  final private[freearrow] case class First[F[_, _], A, B, C](_first: FAC[F, A, B]) extends FAC[F, (A, C), (B, C)] {
    override def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[(A, C), (B, C)] = A.first(_first.foldMap(fk))
  }
  final private[freearrow] case class Second[F[_, _], A, B, C](_second: FAC[F, A, B]) extends FAC[F, (C, A), (C, B)] {
    override def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[(C, A), (C, B)] = A.second(_second.foldMap(fk))
  }
  final private[freearrow] case class Choose[F[_, _], A, B, C, D](_left: FAC[F, A, B], _right: FAC[F, C, D]) extends FAC[F, Either[A, C], Either[B, D]] {
  ...
  final private[freearrow] case class Left[F[_, _], A, B, C](_left: FAC[F, A, B]) extends FAC[F, Either[A, C], Either[B, C]] {
    override def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[Either[A, C], Either[B, C]] = A.left(_left.foldMap(fk))
  }
  final private[freearrow] case class Right[F[_, _], A, B, C](_right: FAC[F, A, B]) extends FAC[F, Either[C, A], Either[C, B]] {
    override def foldMap[G[_, _]](fk: F ~~> G)(implicit A: ArrowChoice[G]): G[Either[C, A], Either[C, B]] = A.right(_right.foldMap(fk))
  }
  final private[freearrow] case class Choice[F[_, _], A, B, C](_left: FAC[F, A, B], _right: FAC[F, C, B]) extends FAC[F, Either[A, C], B] {
