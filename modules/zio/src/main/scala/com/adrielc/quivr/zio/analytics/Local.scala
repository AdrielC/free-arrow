  ...
  implicit class ZStreamOps[R, E, A](self: ZStream[R, E, A]) {
    ...
    final def groupByOrBroadcast[R1 <: R, E1 >: E, K, V](
      ...
      val qstream = ZStream.unwrapManaged {
        for {
          decider <- Promise.make[Nothing, (Option[K], V) => UIO[UniqueKey => Boolean]].toManaged_
          out <- Queue.bounded[Take[E1, (K, GroupBy.DequeueOnly[Take[E1, V]])]](buffer).toManaged(_.shutdown)
  ...
  def evalStream[A](ds: DataStream[A]): Stream[Nothing, Record[A]] =
    ds match {
      case DataStream.Literals(data) =>
        Stream.fromChunk(data).map(e => Element(evalExpr(e)(()))) ++ Stream.succeed(Watermark(Long.MaxValue))
      ...
      case ds: DataStream.Fold[k, v, A] =>
        ...
        evalStream(ds.ds).groupByOrBroadcast {
          case w @ Watermark(_) => ZIO.succeed(None          -> w)
          case Element(rec)     => ZIO.succeed(Some(rec.key) -> Element(rec.value))
        } { (k, vs) =>
          ZStream.fromEffect(
            vs.collect {
              case Element(v) => v
      ...
      case ds: DataStream.FoldWindow[k, v, s] =>
        ...
        stream.mapConcat {
          ...
          case w @ Watermark(_) =>
            ZIO.succeed(None -> w)
          case Element(g) =>
            ZIO.succeed(Some(g.key) -> Element(g.value))
        } {
          case ((window, k), vs) =>
            vs.mapAccum(z(()) -> -1L) {
