...
object DataStream {
  ...
  case class Literals[A](data: Chunk[Expression[Unit, A]])
    extends DataStream[A]
  case class Map[A, B](ds: DataStream[A], f: Expression[A, B])
    extends DataStream[B]
  case class MapConcat[A, B](ds: DataStream[A], f: Expression[A, List[B]])
    extends DataStream[B]
  case class Filter[A](ds: DataStream[A], f: Expression[A, Boolean])
    extends DataStream[A]
  ...
  case class Fold[K, V, R](ds: DataStream[Grouped[K, V]], f: Expression[Group[K, V], R])
    extends DataStream[R]
  case class MapValues[K, V, B](ds: DataStream[Grouped[K, V]], f: Expression[V, B])
    extends DataStream[Grouped[K, B]]
