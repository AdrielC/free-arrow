...
import cats.{Alternative, Applicative, ApplicativeError, Apply, CommutativeFlatMap, CommutativeMonad, Contravariant, ContravariantMonoidal, Defer, Distributive, FlatMap, Functor, FunctorFilter, Monad, MonadError, Monoid, MonoidK, Parallel, Representable, Semigroup, SemigroupK, Traverse, ~>}
import cats.arrow._

/**
 * Represents a function `A => F[B]`.
 * Copy of [[cats.data.Kleisli]] with covariant [[F]]
 */
...
sealed abstract private[data] class KleisliInstances extends KleisliInstances0 {
  implicit def catsDataMonadForKleisliId[A]: CommutativeMonad[Kleisli[λ[+[α] => α], A, *]] =
    catsDataCommutativeMonadForKleisli[λ[+[α] => α], A]

  implicit val catsDataCommutativeArrowForKleisliId: CommutativeArrow[Kleisli[λ[+[α] => α], *, *]] =
    catsDataCommutativeArrowForKleisli[λ[+[α] => α]]
...
sealed abstract private[data] class KleisliInstances1 extends KleisliInstances2 {
  ...
  implicit def catsDataParallelForKleisli[M[+_], P[+_], A](
    implicit P: Parallel.Aux[M, P]
  ): Parallel.Aux[Kleisli[M, A, +*], Kleisli[P, A, *]] = new Parallel[Kleisli[M, A, +*]] {
    type F[+x] = Kleisli[P, A, x]
    implicit val monadM: Monad[M] = P.monad
    def applicative: Applicative[Kleisli[P, A, *]] = catsDataApplicativeForKleisli[P, A](P.applicative)
    def monad: Monad[Kleisli[M, A, *]] = catsDataMonadForKleisli

    def sequential: Kleisli[P, A, *] ~> Kleisli[M, A, *] =
      λ[Kleisli[P, A, *] ~> Kleisli[M, A, *]](_.mapK(P.sequential))

    def parallel: Kleisli[M, A, *] ~> Kleisli[P, A, *] =
      λ[Kleisli[M, A, *] ~> Kleisli[P, A, *]](_.mapK(P.parallel: M ~> P))
  }
...
sealed abstract private[data] class KleisliInstances3 extends KleisliInstances4 {
  ...
  implicit val catsDataChoiceForKleisliId: Choice[Kleisli[λ[+[α] => α], *, *]] =
    catsDataChoiceForKleisli[λ[+[α] => α]]
