...
object ResultRelevancies extends ResultRels {
  ...
  def apply[A, Rel](implicit R: ResultRelevancies.Aux[A, Rel], dummyImplicit: DummyImplicit): ResultRelevancies.Aux[A, Rel] = R
...
trait ResultRels {
  ...
  implicit val contravariantRels0: Contravariant[ResultRelevancies] = new Contravariant[ResultRelevancies] {
    def contramap[A, B](fa: ResultRelevancies[A])(f: B => A): ResultRelevancies[B] = new ResultRelevancies[B] {
      override type Rel = fa.Rel
      override val rel: Relevancy[Rel] = fa.rel
      def resultRelevancies(a: B): NonEmptyVector[Rel] = fa.resultRelevancies(f(a))
    }
  }
}
