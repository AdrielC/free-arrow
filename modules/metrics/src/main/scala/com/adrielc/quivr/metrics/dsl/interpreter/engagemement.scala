...
object engagemement {
  ...
  val labelerCompiler: engagement.Labeler ~> FromEngs[*, Double] = {
    new (engagement.Labeler ~> λ[α => Kleisli[Option, Map[α, Int], Double]]) {
      def apply[A](fa: engagement.Labeler[A]): Kleisli[Option, Map[A, Int], Double] =
        fa.cata[FromEngs[A, Double]] {
          ...
          case i: IfThen[A, FromEngs[A, Double]] => judgeCompiler(i.i).flatMap(b => if(b) i.t else Kleisli((_: Map[A, Int]) => none[Double]))
          case Or(e1, e2)       => e1 <+> e2
  ...
  val judgeCompiler: engagement.Judge ~> FromEngs[*, Boolean] = {
  ...
  private val orElseZeroCompiler: engagement.Labeler ~> FromEngs[*, Double] =
    λ[engagement.Labeler ~> FromEngs[*, Double]](fa => labelerCompiler(fa) <+> Kleisli.pure(0.0))
