...
object engagement {
  ...
  object Labeler {
    def count[E](e: E): Labeler[E] = Fix(Count(e): LabelerF[E, Labeler[E]])

    def doubleVal[E](v: Double): Labeler[E] = Fix(Value(v): LabelerF[E, Labeler[E]])

    def sum[E](e1: Labeler[E], e2: Labeler[E]): Labeler[E] = Fix(Sum(e1, e2))

    def log[E](e1: Labeler[E], base: Double): Labeler[E] = Fix(Log(e1, base))

    def mult[E](e1: Labeler[E], e2: Labeler[E]): Labeler[E] = Fix(Mult(e1, e2))
  ...
  private[dsl] object LabelerF {
    ...
    implicit def functorLabelerF[E]: Functor[LabelerF[E, *]] =
      new Functor[LabelerF[E, *]] {
        def map[A, B](fa: LabelerF[E, A])(f: A => B): LabelerF[E, B] = fa.map(f)
      }
  }
  ...
  private[dsl] object JudgeF {
    ...
    implicit def functorJudgeF[E]: Functor[JudgeF[E, *]] =
      new Functor[JudgeF[E, *]] {
        def map[A, B](fa: JudgeF[E, A])(f: A => B): JudgeF[E, B] = fa.map(f)
      }
  }
  ...
  trait LabelFor[A, E] {
    def labeler(a: A): Labeler[E]
  }

  object LabelFor {
    implicit def liftFixExpr[E]: Labeler[E] LabelFor E = (a: Labeler[E]) => a

    implicit def liftNumeric[N, E](implicit N: Numeric[N]): N LabelFor E = a => Fix(Value(N.toDouble(a)): LabelerF[E, Labeler[E]])
  }

  implicit class LabelForOps[A](private val a: A) extends AnyVal {
