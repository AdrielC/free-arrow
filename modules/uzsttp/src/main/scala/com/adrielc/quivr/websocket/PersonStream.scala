package com.adrielc.quivr.websocket
...
import com.adrielc.quivr.uzsttp.servers.Person._
import com.adrielc.quivr.uzsttp.servers.Encoders._
import com.adrielc.quivr.uzsttp.servers.Person
import zio.stream._
import com.adrielc.quivr.uzsttp.servers.hrequest._
import com.adrielc.quivr.uzsttp.servers.EndPoint._
...
object PersonStream {
  ...
  val agePersonByOne: EndPoint[HRequest] =
    for {
      ...
      streamOut: Stream[Throwable, Stream[HTTPError, Frame]] = req.frames
        .map(handleWebsocketFrame((string, _) => agePerson(string)))
...
case class PersonStream(clk: Clock.Service) {
  ...
  val agePerson: EndPoint[HRequest] =
    for {
      ...
      streamOut = req.frames.map(handleWebsocketFrame((string, _) => autoAgeText(string))).flatten
